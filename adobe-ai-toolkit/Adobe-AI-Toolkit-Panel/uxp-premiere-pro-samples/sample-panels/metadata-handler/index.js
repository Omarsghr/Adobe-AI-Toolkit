...
function toggleIcon() {
  ...
  // CSP-friendly approach: avoid innerHTML assignment of SVG strings.

  const updateBtn = document.getElementById("column-update-button");
  if (!updateBtn) return;

  const isCopy = document.getElementById("copy")?.checked;
  const svgString = isCopy ? copySvg : exchangeSvg;

  // Clear existing content
  while (updateBtn.firstChild) {
    updateBtn.removeChild(updateBtn.firstChild);
  }

  // Parse SVG into DOM nodes (no eval/new Function)
  const parser = new DOMParser();
  const doc = parser.parseFromString(svgString, "image/svg+xml");
  const svgEl = doc.documentElement;

  updateBtn.appendChild(svgEl);
}
